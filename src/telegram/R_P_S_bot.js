                      ...
                      if (err) console.log("sqlerror: ", err);
                        ...
                        if (rooms.length) {
                          ...
                          if (rooms[0].user_count < 2) {
                            // const userId = ctx.from.id;
                            // console.log(userId);
                            // const wallet = new ethers.Wallet(existingWallet.privateKey, provider);
                            // const nonce = await provider.getTransactionCount(existingWallet.address);
                            // const existingWallet = await getWalletDetails(userId);
                            // const tx = {
                            //   nonce: nonce,
                            //   gasLimit: ethers.utils.hexlify(210000), // Gas limit
                            //   gasPrice: ethers.utils.parseUnits("1", "gwei"), // Gas price in gwei
                            //   to: contractAddress, // Address of the contract
                            //   value: ethers.utils.parseEther(rooms[0].player1_amount), // Amount in ether
                            //   data: contract.interface.encodeFunctionData("joinRoom", [roomName]) // Function data
                            // };
